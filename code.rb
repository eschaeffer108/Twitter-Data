...
#YOUR CODE GOES HERE
twitter_data.each do |name|
  print name.keys[0] + ": "
  name.each do |username, values|
    if values["description"].nil?
      puts "N/A"
    else puts values["description"]
    end
  end
end
...
twitter_data.each do |name|
  print name.keys[0] + ": "
  name.each do |username, values|
    puts values["number of followers"]
  end
end

twitter_data.each do |name|
  name.each do |username, values|
    puts name.keys[0] +  "latest tweet was #{values["latest tweet"].length} characters."
  end
end
...
twitter_data.each do |name|
  name.each do |username, values|
    values["last twenty tweets"].each do |tweet|
      char_count += tweet.length
    end
    puts name.keys[0] + "last twenty tweets had #{char_count} total characters."
  end
end
...
most_popular = nil
twitter_data.each do |name|
  name.each do |name, values|
    if values["number of followers"] > most_followers
    most_followers = values["number of followers"]
    most_popular = name
  end
end
end
puts "#{most_popular} has #{most_followers} followers!"
...
twitter_data.each do |name|
  name.each do |name, values|
    if values["number of friends"] > most_friends
      most_friends = values["number of friends"]
      most_likeable = name
    end
  end
end
puts "#{most_likeable} has #{most_friends} friends!"
...
twitter_data.each do |name|
  name.each do |name, values|
    if values["number of tweets"] > most_tweets
      most_tweets = values["number of tweets"]
      biggest_tweeter = name
    end
  end
end
puts "#{biggest_tweeter} tweeted #{most_tweets} tweets!"
...
twitter_data.each do |name|
  name.each do |name, values|
    if !values["location"].nil?
      users_with_location << (name)
    end
  end
end
puts "Has a location:"
users_with_location.each do |name|
  puts name
end

puts "Does not have a location:"
